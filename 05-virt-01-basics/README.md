...
Основная разница в изоляции и необходимости модифицировать гостевые ОС:
- при полной виртуализации гипервизор обеспечивает лучшую изоляцию операционных систем друг от друга, не требуется установка ОС на хост т.к. сам гипервизор является ОС, не требуется модификация гостевой ОС;
- при паравиртуализации необходима установка ОС на хост и модификация ядра и драйверов гостевой ОС;
- при виртуализации на основе ОС, плохая изоляция, гостевая ОС не имеет собственного ядра, а использует ядро хоста.  
...
- можно купить поддержки у вендоров;
- если гетерогенность не оправдана, то рассмотреть возможность отказа от нее;
- рассмотреть варианты использования IaaS, PaaS, SaaS, инфраструктуру вывести в IaC (Terraform);
- максимальное автоматизировать развертывание и тестирование инфраструктуры, чтобы она была единая.

Я бы предпочел работать в Cloud среде таких как AWS, GCP. Небольшие выгоды в цене и производительности при использовании гетерогенных сред не всегда оправданы.
